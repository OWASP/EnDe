...
#
#? VERSION
#?      @(#) Makefile 3.40 12/11/23 23:14:14
#?
#? AUTHOR
...
$(SRC.rel): $(SCCS_SRC)
	@(\
	  ...
	  echo "# created at " `date`  by Makefile 3.40; \
	  echo "#"; \
	  echo "# filename                       SID"; \
	  echo "# ------------------------------+-----------"; \
...
$(GEN.SID): $(SCCS_SRC)
	@(\
	  ...
	  perl -ne 'printf("  \"%s\" : \"%s\",\n",$$1,$$2) if m/(?:\s+|content=")@\(#\)\s*([^ ]*)\s*([^ ]*)/;' $(SCCS_SRC) ; \
	  echo ''; \
	  echo '  "SID" : "EnDeGUI.SIDs generated by Makefile 3.40 : make $@"\n};'; \
	) > $@
...
$(GEN.map): $(SRC.map)
	@(\
	  echo '/*';  \
	  echo ' * created at ' `date` from $< by Makefile 3.40;  \
	  echo ' */'; \
	  echo 'EnDe.MAPS = "\\n"+'; \
...
GEN.ZAP = $(SRC.lib:%=zap/%) $(SRC.zap:%=zap/%)
