...
#
#? VERSION
#?      @(#) Makefile 3.46 20/12/13 14:28:51
#?
#? AUTHOR
...
# --------------------------------------------------------------------- sources
# NOTE: When using JavaScript library, the sequence of files are important to
#       avoid undefined objects. As this library is build mainly by `cat'ting
#       files together, the sequence of files in the  SRC.func  and  SRC.lib 
#       are important.
SRC.func = \
        EnDe.js \
	aes.js \
	...
	EnDeIP.js \
	EnDeTS.js
SRC.map = EnDeMaps.txt
GEN.map = EnDeMaps.gen.js
SRC_lib = \
	$(SRC.func) \
	$(GEN.map) \
        EnDeMaps.js
SRC.zap = EnDe.zap.js
#GEN.zap = # see below: targets
SRC.lib = $(SRC_lib)
GEN.lib = EnDeFunc.js
...
othergui= JsColorizer.js JsDecoder.js JSReg.js vkbeautify.js
...
SRC_src = \
	...
	core-rules-2.2.8.0.xml \
	core-rules-2.2.8.0-part1.xml \
	core-rules-2.2.8.0-part2.xml \
	core-rules-2.2.8.0-part3.xml \
	sqlPattern.txt \
	xssAttacks.xml \
	xss.h4k.xml \
	xss.mario.xml \
...
SRC_3rd	= \
	...
	owasp-modsecurity-crs-2.2.8-0-g0f07cbb.tar.gz \
	CoreRules2Html.jar \
	JsDecoder.html \
	JsDecoder.js \
	JsColorizer.js \
...
#SCCS.SRC += $(SRC.img)
GEN.ALL = $(GEN.lib) $(GEN.map) $(GEN.gui) $(GEN.src) $(GEN.html) $(GEN.tst) $(GEN.doc) $(GEN.SID) $(GEN.zap)
...
$(LINE)\n\
    ...
    gen.zap      - generate $(GEN.zap)	\n\
    zap          - generate $(GEN.zap)	\n\
...
gen.func: EnDeFunc.html EnDeFunc.js EnDeFunc.txt EnDeFunc.xml

EnDeFunc.html:: $(ende2html) $(SRC.func) Makefile
	@$(MAKE) -s gen.func=$(ende2html) -s gen.file=$@ _gen.func

EnDeFunc.js:: $(ende2js) $(SRC.func) Makefile
	@$(MAKE) -s gen.func=$(ende2js)   -s gen.file=$@ _gen.func

EnDeFunc.txt:: $(ende2txt) $(SRC.func) Makefile
	@$(MAKE) -s gen.func=$(ende2txt)  -s gen.file=$@ _gen.func

EnDeFunc.xml:: $(ende2xml) $(SRC.func) Makefile
	@$(MAKE) -s gen.func=$(ende2xml)  -s gen.file=$@ _gen.func
...
$(SRC.rel): $(SCCS_SRC)
	@(\
	  ...
	  echo "# created at " `date`  by Makefile 3.46; \
	  echo "#"; \
	  echo "# filename                       SID"; \
	  echo "# ------------------------------+-----------"; \
...
$(GEN.SID): $(SCCS_SRC)
	@(\
	  ...
	  perl -ne 'printf("  \"%s\" : \"%s\",\n",$$1,$$2) if m/(?:\s+|content=")@\(#\)\s*([^ ]*)\s*([^ ]*)/;' $(SCCS_SRC) ; \
	  echo ''; \
	  echo '  "SID" : "EnDeGUI.SIDs generated by Makefile 3.46 : make $@"\n};'; \
	) > $@

# convert TAB-separated text file to JavaScript variable contaning text
$(GEN.map): $(SRC.map)
	@(\
	  echo '/*';  \
	  echo ' * created at ' `date` from $< by Makefile 3.46;  \
	  echo ' */'; \
	  echo 'EnDe.MAPS = "\\n"+'; \
	  sed -ne 's/\\/\\\\/g' -e 's/"/\\"/g' -e '/^__DATA/,/__END/s/.*/"&\\n"+/p' $< ; \
	  echo '""; // EnDe.MAPS'; \
	) > $@
...
lib.js  = $(PROJECT).lib.js
lib.zap = $(PROJECT).lib.zap
lib.tgz = $(PROJECT).lib.tgz

GEN.zap = $(lib.zap)
GEN.ZAP = $(SRC.lib:%=zap/%) $(SRC.zap:%=zap/%) zap/HELP.js
...
zap/%:: %
	$(pack.BIN) $< $(pack.OPTo) $@
zap/HELP.js: zap-tmp EnDeFunc.txt
	@(\
	  echo "if (typeof(EnDe.Func)==='undefined'){EnDe.Func=new function() {};};"; \
	  echo 'EnDe.Func.HELP=['; \
	  awk -F"	" '/^item3/{gsub(/\\/,/\\\\/);gsub(/"/,/\\"/); printf("\"%s\\n\\t%s\\n\",\n",$$2,$$4);}' EnDeFunc.txt ; \
	  echo '"" ];'; \
	  echo "EnDe.Func.help=function(str) {"; \
	  echo "	var rex=new RegExp(EnDe.rex(str),'');"; \
	  echo "	var bux='';"; \
	  echo "	for (x in EnDe.Func.HELP) {"; \
	  echo "		if (EnDe.Func.HELP[x].match(rex)!==null){bux+=EnDe.Func.HELP[x];}"; \
	  echo "	}"; \
	  echo "	return bux;"; \
	  echo "};"; \
	) > $@
