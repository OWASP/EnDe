...
#
#? VERSION
#?      @(#) Makefile 3.35 12/06/09 20:37:08
#?
#? AUTHOR
...
SRC_lib = \
	...
	EnDeMaps.txt
SRC.lib = $(SRC_lib)
GEN.lib = EnDeFunc.js EnDeFunc.html
...
$(SRC.rel): $(SCCS_SRC)
	@(\
	  ...
	  echo "# created at " `date`  by Makefile 3.35; \
	  echo "#"; \
	  echo "# filename                       SID"; \
	  echo "# ------------------------------+-----------"; \
...
$(GEN.SID): $(SCCS_SRC)
	@(\
	  ...
	  perl -ne 'printf("  \"%s\" : \"%s\",\n",$$1,$$2) if m/(?:\s+|content=")@\(#\)\s*([^ ]*)\s*([^ ]*)/;' $(SCCS_SRC) ; \
	  echo ''; \
	  echo '  "SID" : "EnDeGUI.SIDs generated by Makefile 3.35 : make $@"\n};'; \
	) > $@
