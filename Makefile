...
#
#? VERSION
#?      @(#) Makefile 3.42 12/12/09 02:42:20
#?
#? AUTHOR
...
#SCCS.SRC += $(SRC.img)
GEN.ALL = $(GEN.lib) $(GEN.map) $(GEN.gui) $(GEN.src) $(GEN.html) $(GEN.tst) $(GEN.doc) $(GEN.SID)
...
$(SRC.rel): $(SCCS_SRC)
	@(\
	  ...
	  echo "# created at " `date`  by Makefile 3.42; \
	  echo "#"; \
	  echo "# filename                       SID"; \
	  echo "# ------------------------------+-----------"; \
...
$(GEN.SID): $(SCCS_SRC)
	@(\
	  ...
	  perl -ne 'printf("  \"%s\" : \"%s\",\n",$$1,$$2) if m/(?:\s+|content=")@\(#\)\s*([^ ]*)\s*([^ ]*)/;' $(SCCS_SRC) ; \
	  echo ''; \
	  echo '  "SID" : "EnDeGUI.SIDs generated by Makefile 3.42 : make $@"\n};'; \
	) > $@
...
$(GEN.map): $(SRC.map)
	@(\
	  echo '/*';  \
	  echo ' * created at ' `date` from $< by Makefile 3.42;  \
	  echo ' */'; \
	  echo 'EnDe.MAPS = "\\n"+'; \
