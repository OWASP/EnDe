...
#
#? VERSION
#?      @(#) Makefile 3.45 14/11/06 21:22:32
#?
#? AUTHOR
...
$(SRC.rel): $(SCCS_SRC)
	@(\
	  ...
	  echo "# created at " `date`  by Makefile 3.45; \
	  echo "#"; \
	  echo "# filename                       SID"; \
	  echo "# ------------------------------+-----------"; \
...
$(GEN.SID): $(SCCS_SRC)
	@(\
	  ...
	  perl -ne 'printf("  \"%s\" : \"%s\",\n",$$1,$$2) if m/(?:\s+|content=")@\(#\)\s*([^ ]*)\s*([^ ]*)/;' $(SCCS_SRC) ; \
	  echo ''; \
	  echo '  "SID" : "EnDeGUI.SIDs generated by Makefile 3.45 : make $@"\n};'; \
	) > $@
...
$(GEN.map): $(SRC.map)
	@(\
	  echo '/*';  \
	  echo ' * created at ' `date` from $< by Makefile 3.45;  \
	  echo ' */'; \
	  echo 'EnDe.MAPS = "\\n"+'; \
