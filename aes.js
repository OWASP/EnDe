...
#?          2. EnDe.AES.EN. and EnDe.AES.DE. wrapper object
#?          3. escCtrlChars() and unescCtrlChars() depends on EnDe.AES.escCtrl
#?          Two semicolons for pedantic JavaScript syntax added.
...
function escCtrlChars(str) {  // escape control chars which might cause problems handling ciphertext
  if (EnDe.AES.escCtrl == false) { return str; }
...
function Cipher(input, w) {    // main Cipher function [§5.1]
  ...
  for (var x in state){ state[x].length = 0; }; state.length = 0; // #mem#
  return output;
}
...
function AESEncryptCtr(plaintext, password, nBits) {
  ...
  pwBytes.length = 0;      // #mem#
  counterBlock.length = 0; // #mem#
  nonce = null;            // #mem#
